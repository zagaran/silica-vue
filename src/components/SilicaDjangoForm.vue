  ...
  watch: {
    data: {
      handler(newData, oldData) {
        if (!_.isEqual(oldData, newData) && this.loadComplete) {
          this.data_ = newData;
        }
      }, deep: true
    },
    schema: {
      handler(newSchema, oldSchema) {
        if (!_.isEqual(oldSchema, newSchema) && this.loadComplete) {
    ...
    uischema: {
      handler(newUischema, oldUischema) {
        if (!_.isEqual(oldUischema, newUischema) && this.loadComplete) {
    ...
    djangoErrors: {
      handler(newErrors, oldErrors) {
        if (!_.isEqual(oldErrors, newErrors) && this.loadComplete) {
    ...
    ingestData(dataKey, objToCopy) {
      ...
      if (!_.isEmpty(objToCopy)) {
        ...
        for (let key of Object.keys(preppedData)) {
          console.log('key', key)
          _.setWith(self[dataKey], key, preppedData[key], (nsValue, key, nsObject) => {
            console.log(nsObject, key, nsValue)
            return self.$set(nsObject, key, nsValue)
          })
        }
      }
    }
